...
void bst_delete(bst& b, bnode* n){
	...
	{ //Entrambi i figli: cerco l'elemento più grande nel sottoalbero di sx
...
void bst_print(bst albero)
{
	if (albero == NULL)
		return;
	
	bst_print(get_left(albero));
	print(albero->inf);
	std::cout << std::endl;
	bst_print(get_right(albero));
}
